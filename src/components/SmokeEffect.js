 ...
 * 
 * A WebGL-based fluid simulation that creates a beautiful smoke/fluid effect
 * following the user's cursor movement. The effect uses:
 * - WebGL2 for rendering (falls back to WebGL1 if unavailable)
 * - Navier-Stokes equations for fluid dynamics simulation
 * - Multi-colored smoke trails that follow cursor movement
 * 
 * The canvas has pointer-events: none to not interfere with underlying UI elements
 * while still tracking mouse movements via window event listeners.
 */
export default function SmokeEffect() {
  ...
  useEffect(() => {
    ...
    const initFluid = async () => {
      ...
      // Mouse events - Listen on window instead of canvas since canvas has pointer-events: none
